...
const {
  ...
  updatePerfil,
} = require('../../Services/mailerServices');
...
function validatePassword(password) {
  return /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&#.])[A-Za-z\d@$!%*?&#.]{6,10}$/.test(
    password
  );
}
...
async function postUsers(req, res) {
  try {
    ...
    if (name && !validateName(name)) {
      errores.push(
        'El nombre debe ser alfanumérico y tener entre 3 y 15 caracteres'
      );
    }
    if (lastName && !validateLastName(lastName)) {
      errores.push(
        'El apellido debe ser alfanumérico y tener entre 3 y 15 caracteres'
      );
    }
    ...
    if (password && !validatePassword(password)) {
      errores.push(
        'La contraseña debe tener entre 6 y 10 caracteres, al menos una letra mayúscula, una letra minúscula, un dígito y un carácter especial'
      );
    }

    if (errores.length > 0) {
      return res.status(401).json({ 'ERROR:': errores });
    }
...
async function putUsers(req, res) {
  try {
    const { id_user } = req.params;
    const { name, email, password, isActive, lastName, contact, img } =
      req.body;
